...
def test_the_export_carries_the_whole_setup(workbook):
    assert workbook.sheetnames == ["Project", "Workflow", "Trades", "Sections",
                                   "Document kinds", "Deliverables"]
...
def test_a_feeding_line_survives_the_round_trip(workbook):
    """Whether a line hands anything over travels with the workbook, or a team
    editing offline would quietly turn every feeder back into a deliverable."""
    ws = workbook["Deliverables"]
    header = [c.value for c in ws[1]]
    assert header[-1] == "Submits"
    assert all(r[len(header) - 1] == "yes"
               for r in ws.iter_rows(min_row=2, values_only=True) if r[2])
